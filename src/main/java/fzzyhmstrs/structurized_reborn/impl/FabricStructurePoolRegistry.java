...
public class FabricStructurePoolRegistry {
    ...
    private static final ResourceKey<StructureProcessorList> EMPTY_PROCESSOR_KEY = ResourceKey.create(Registries.PROCESSOR_LIST, Identifier.withDefaultNamespace("empty"));
    ...
    public static void registerSimple(Identifier poolId, Identifier structureId, int weight) {
        register(poolId, structureId, weight, EMPTY_PROCESSOR_KEY, StructureTemplatePool.Projection.RIGID, StructurePoolElementType.LEGACY);
    }
    ...
    public static void registerFeature(Identifier poolId, Identifier structureId, int weight, StructureTemplatePool.Projection projection, Holder<PlacedFeature> entry) {
        register(poolId, structureId, weight, EMPTY_PROCESSOR_KEY, projection, StructurePoolElementType.FEATURE);
    ...
    public static void registerList(Identifier poolId, int weight, ListPoolElement listPoolElement) {
        register(poolId, Identifier.withDefaultNamespace("air"), weight, EMPTY_PROCESSOR_KEY, StructureTemplatePool.Projection.RIGID, StructurePoolElementType.LIST);
    ...
    public static void processRegistry(FabricStructurePool structurePool, HolderGetter<StructureProcessorList> registryEntryLookup) {
    ...
    private static void addToPool(FabricStructurePool structurePool, Quintuple<String, String, ResourceKey<StructureProcessorList>, String, Integer> quint, String key, HolderGetter<StructureProcessorList> registryEntryLookup) {
        ...
        if (type == StructurePoolElementType.SINGLE) {
            ...
            Holder<StructureProcessorList> entry = registryEntryLookup.getOrThrow(EMPTY_PROCESSOR_KEY);
            spe.add(StructurePoolElement.legacy("minecraft:air", entry).apply(StructureTemplatePool.Projection.RIGID));
        }
